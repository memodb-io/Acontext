...
All workflows create a GitHub Release with a path-scoped changelog generated by `.github/scripts/generate-changelog.sh`. The script finds the previous tag with the same prefix, collects commits scoped to the component's source directory, and groups them by conventional commit type (Features, Bug Fixes, Other). Docker builds produce multi-platform images (`linux/amd64`, `linux/arm64`). npm/PyPI workflows skip publishing if the version already exists on the registry. npm packages use OIDC trusted publishers for authentication — do NOT add `NODE_AUTH_TOKEN` or npm tokens to the publish step.
### Pull Requests
