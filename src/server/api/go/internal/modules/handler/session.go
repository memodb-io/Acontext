...
type StoreMessageReq struct {
	...
	Format string      `form:"format" json:"format" binding:"omitempty,oneof=acontext openai anthropic genai" example:"openai" enums:"acontext,openai,anthropic,genai"`
}
...
func (h *SessionHandler) StoreMessage(c *gin.Context) {
	...
	case model.FormatGenAI:
		// Parse and validate using official Google GenAI SDK
		norm := &normalizer.GenAINormalizer{}
		normalizedRole, normalizedParts, normalizedMeta, err = norm.NormalizeFromGenAIMessage(blobJSON)
		if err != nil {
			c.JSON(http.StatusBadRequest, serializer.ParamErr("failed to normalize GenAI message", err))
			return
		}
...
type GetMessagesReq struct {
	...
	Format             string `form:"format,default=openai" json:"format" binding:"omitempty,oneof=acontext openai anthropic genai" example:"openai" enums:"acontext,openai,anthropic,genai"`
	TimeDesc           bool   `form:"time_desc,default=false" json:"time_desc" example:"false"`
...
//	@Param			format					query	string	false	"Format to convert messages to: acontext (original), openai (default), anthropic, genai."	enums(acontext,openai,anthropic,genai)
//	@Param			time_desc				query	string	false	"Order by created_at descending if true, ascending if false (default false)"		example(false)
