...
from .implementation import CloudflareSandboxRuntime, DockerSandboxRuntime


_RUNTIME_REGISTRY: Dict[SandboxBackend, Callable[[SandboxInfo], SandboxRuntime]] = {
    SandboxBackend.DOCKER: lambda sandbox: DockerSandboxRuntime(sandbox.id),
    SandboxBackend.CLOUDFLARE: lambda sandbox: CloudflareSandboxRuntime(sandbox.id),
}
