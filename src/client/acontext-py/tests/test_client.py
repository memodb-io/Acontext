...
@patch("acontext.client.AcontextClient.request")
def test_add_event_disk_event(mock_request, client: AcontextClient) -> None:
    """Test add_event with DiskEvent."""
    from acontext.event import DiskEvent

    mock_request.return_value = MOCK_SESSION_EVENT

    event = client.sessions.add_event(
        "session-uuid",
...
@patch("acontext.client.AcontextClient.request")
def test_add_event_text_event(mock_request, client: AcontextClient) -> None:
    """Test add_event with TextEvent."""
...
@patch("acontext.client.AcontextClient.request")
def test_get_events_with_pagination(mock_request, client: AcontextClient) -> None:
    ...
    result = client.sessions.get_events("session-uuid", limit=1)
    ...
    assert len(result.items) == 1
    assert result.items[0].type == "disk_event"
    assert result.has_more is True
    assert result.next_cursor == "cursor-abc"


@patch("acontext.client.AcontextClient.request")
def test_get_messages_with_events(mock_request, client: AcontextClient) -> None:
    """Test get_messages with with_events=True."""
    mock_request.return_value = {
        "items": [{"role": "user", "content": "Hello"}],
        "ids": ["msg-1"],
        "metas": [{}],
    ...
    result = client.sessions.get_messages("session-uuid", with_events=True)
...
def test_disk_event_validation() -> None:
    """Test DiskEvent validates required fields."""
    from acontext.event import DiskEvent
    import pytest as _pytest

    with _pytest.raises(Exception):
        DiskEvent()  # type: ignore


def test_text_event_validation() -> None:
    """Test TextEvent validates required fields."""
    from acontext.event import TextEvent
    import pytest as _pytest

    with _pytest.raises(Exception):
        TextEvent()  # type: ignore
