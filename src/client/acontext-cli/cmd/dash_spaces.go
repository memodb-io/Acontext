...
func init() {
	...
	getCmd := &cobra.Command{
		Use: "get <space-id>", Short: "Get learning space details", Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			c, err := requireClient()
			if err != nil {
				return err
	...
	deleteCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !yes {
				...
				proceed, err := tui.RunConfirm(fmt.Sprintf("Delete learning space %s?", args[0]), false)
				if err != nil || !proceed {
	...
	spacesCmd.AddCommand(listCmd, getCmd, createCmd, deleteCmd, learnCmd)
