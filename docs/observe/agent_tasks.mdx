...
# Poll until tasks are extracted (with timeout)
max_attempts = 10
tasks_response = None
for attempt in range(max_attempts):
    tasks_response = client.sessions.get_tasks(session.id)
    if len(tasks_response.items) > 0:
        break
    time.sleep(1)  # Wait 1 second between attempts

if not tasks_response or len(tasks_response.items) == 0:
    print("No tasks extracted yet, extraction may still be in progress")
else:
    print(f"Successfully extracted {len(tasks_response.items)} tasks!")
...
async function demonstrateTasks() {
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
        tasksResponse = await client.sessions.getTasks(session.id);
