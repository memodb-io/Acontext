...
4. **Skill Storage**: Learned skills and user preferences are stored in the Space as searchable knowledge blocks
...
# Step 1: Create a Space for skill learning (optionally associate with a user)
space = client.spaces.create(user="user@example.com")
print(f"Created Space: {space.id}")

# Step 2: Create a session attached to the space (optionally associate with a user)
session = client.sessions.create(space_id=space.id, user="user@example.com")
...
async function demonstrateSkillLearning() {
    // Step 1: Create a Space for skill learning (optionally associate with a user)
    const space = await client.spaces.create({ user: 'user@example.com' });
...
<Tip>
You can associate spaces and sessions with a specific user by passing the `user` parameter when creating them. This enables per-user skill libraries and resource cleanup. See [Per-user Resources](/chore/per-user) for details.
</Tip>
