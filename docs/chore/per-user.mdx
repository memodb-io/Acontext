...
User-based resource management allows you to associate Acontext resources (Sessions, Disks, and Skills) with user identifiers. This enables multi-tenant scenarios, per-user resource isolation, and simplified resource cleanup.

## What you'll learn

In this guide, you'll learn how to:

- Associate resources (Sessions, Disks, Skills) with user identifiers
...
Deleting a user removes the user record and cascades to delete all associated resources (Sessions, Disks, and Skills).
...
<Warning>
Deleting a user is a destructive operation that cannot be undone. All Sessions, Disks, and Skills associated with the user will be permanently deleted.
</Warning>
</Step>
</Steps>
...
def main():
    ...
    try:
        ...
        print(f"✓ User has {len(sessions.items)} session(s), {len(disks.items)} disk(s)")
...
async function main() {
    ...
    try {
        ...
        console.log(`✓ User has ${sessions.items.length} session(s), ${disks.items.length} disk(s)`);
...
async def main():
    async with AsyncAcontextClient(
        ...
        session = await client.sessions.create(user="alice@acontext.io")
        sessions = await client.sessions.list(user="alice@acontext.io")
        await client.users.delete("alice@acontext.io")
...
client.disks.create(user="alice@acontext.io")

# Avoid: Inconsistent formats
client.sessions.create(user="alice@acontext.io")
client.disks.create(user="user_123")  # Different format for same user
```
