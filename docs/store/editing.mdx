...
<Info>
The token count of Acontext is relative and proportional to the length of your session. You can use it to determine whether the current session is too long and needs to be addressed. Please do not use the token count to calculate the cost of LLM, as the actual token consumption of each LLM can vary subtly.
</Info>

## Basic Context Editing SDKs
...
client.sessions.context_edit(session_id="session-uuid", strategy="STRATEGY_NAME", params={...})
 
# Edited session
edited_session = client.sessions.get_messages(session_id="session-uuid")
```

### Remove Tool Result
```python Python
client.sessions.context_edit(session_id="session-uuid", strategy="remove_tool_result", params={"keep_recent_n_tool_results": 10})
```
