...
- **`glob`** - Find files by path pattern using glob syntax
...
<Tip>
Use `glob` to find files by extension or location, perfect for discovering files without knowing exact names.
</Tip>
