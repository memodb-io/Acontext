...
Thinking blocks are stored as Anthropic `thinking` content blocks with their `signature` preserved. When retrieved in Anthropic format, they round-trip as native `thinking` blocks. When retrieved in OpenAI or Gemini format, thinking content is automatically downgraded to plain text.
